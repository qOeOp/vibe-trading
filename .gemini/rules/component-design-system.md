...
> 以下是经过筛选的外部灵感来源，按优先级排序。
> **使用方式**：借鉴 CSS 技法和组件组合模式，copy-paste 到项目自有组件中。不安装这些库。

### 10.1 Tier 1 — 核心参考（每次构建新组件前应查阅）

#### Origin UI (coss.com/ui)
- **借鉴价值**：输入框变体、表格增强、微交互
- **关键 CSS 技法 — 伪元素内凹阴影**：
  ```css
  /* 在 input/button/card 上创建微妙的 3D 按压边缘感 */
  before:pointer-events-none before:absolute before:inset-0
  before:rounded-[calc(var(--radius-xl)-1px)]
  before:shadow-[0_1px_rgba(0,0,0,0.04)]
  ```
  这个 `before:` 伪元素在顶部边缘产生 1px 的内凹高光，让扁平元素获得微妙的立体感。适合你的 Mine 主题暖色卡片。
- **值得复刻的组件模式**：
  - 带 `<kbd>` 快捷键提示的搜索输入框
  - 带 +/- 步进器的数值输入框（适合手数、参数输入）
  - 带复制按钮的输入框（适合因子 ID、策略代码分享）
  - 语义化状态 badge：`bg-[color]/8 text-[color]-foreground`（8% 不透明度底色 + 前景色文字）
  - 按钮按压深度：静止态 `inset-shadow-[0_1px_rgba(255,255,255,0.16)]`，按下态反转为 `inset-shadow-[0_1px_rgba(0,0,0,0.08)]`

#### Tremor Raw (tremor.so)
- **借鉴价值**：数据仪表盘专用组件，最贴合金融场景
- **值得复刻的组件模式**：
  - **Tracker**：水平色块条，每块代表一个时间单位，颜色编码状态。用法：因子 IC 30 日表现追踪（每天一个色块，颜色 = IC 五分位）
  - **SparkChart 行内迷你图**：默认 `h-12 w-28`，无轴线，纯趋势可视化。用法：因子卡片中的 IC 趋势缩略图、表格行内的策略净值曲线
  - **BadgeDelta 变化标签**：带方向箭头的 badge，颜色编码正/负变化。映射到 `market-up-medium` / `market-down-medium`
  - **CategoryBar 分段条**：水平分段条显示分布。用法：因子五分位分布、组合行业配比
  - **KPI 卡片组合公式**：`标签(text-sm muted) → 数值(text-3xl semibold) → delta badge + 上下文 → 可选 sparkline`

#### NumberFlow (@number-flow/react)
- **借鉴价值**：数字滚动动画，直接解决"数值变化静态替换"痛点
- **兼容性**：React 18/19，MIT，零依赖
- **推荐集成模式**：
  ```tsx
  // 在 components/ui/ 或 components/shared/ 下创建包装组件
  import NumberFlow from '@number-flow/react'

  function MarketValue({ value, prevValue, format, className }: MarketValueProps) {
    const trend = value > prevValue ? 1 : value < prevValue ? -1 : 0
    return (
      <NumberFlow
        value={value}
        trend={trend}
        ...
        className={cn(
          ...
          trend > 0 && "text-market-up-medium",
          trend < 0 && "text-market-down-medium",
          trend === 0 && "text-market-flat",
          className
        )}
  ...
  ```
- **使用场景**：实时价格显示、组合总值、因子 IC 值、策略收益率、所有 KPI 指标

### 10.2 Tier 2 — 交互模式参考（构建复杂交互时查阅）

#### cult/ui (cult-ui.com)
- **借鉴价值**：渐进式信息披露（progressive disclosure）模式
- **值得研究的模式**：
  - **Floating Panel**：浮动面板 UI，适合因子详情弹出
  - **Expandable Card**：卡片展开为全视图，适合因子卡片点击展开分析
  - **Toolbar Expandable**：上下文感知的可展开工具栏
  - **Direction Aware Tabs**：带方向动画的标签页

#### UI Labs (uilabs.dev)
- **借鉴价值**：成熟的仪表盘微交互设计，与你的技术栈完全一致（React + Tailwind + Framer Motion + Radix）
- **值得研究的模式**：
  - **Contextual Toolbar**：根据导航上下文变化的工具栏，适合因子库筛选
  - **Dynamic Footer Widget**：hover 展示上下文数据的指标组件

#### shadcn/ui blocks (ui.shadcn.com/blocks)
- **借鉴价值**：高层级组合模式，shadcn 官方推荐的 Dashboard 布局
- **关键模式**：
  - Container Query 响应式网格：`@container/main` + `@xl/main:grid-cols-3`，比媒体查询更精准
  - Dashboard 组合层级：KPI stats row → chart area → data table

### 10.3 Tier 3 — 按需查阅（需要特定模式时再看）

| 来源 | 何时查阅 | 值得看什么 |
|------|----------|-----------|
| **blocks.so** | 需要 stat 卡片布局变体时 | 15 个 stats block 模板 |
| **Magic UI** (magicui.design) | 需要 marquee 滚动效果时 | Marquee 组件（市场 ticker 用） |
| **awesome-shadcn-ui** (GitHub) | 需要特定 shadcn 扩展时 | linked-chart（图表联动表格）模式 |

### 10.4 已评估并排除的来源

| 来源 | 排除原因 |
|------|----------|
| DaisyUI | Tailwind v4 不兼容其 plugin 机制，样式偏卡通 |
| HeroUI / NextUI | 玻璃拟态主导，与 Mine 主题浅底设计冲突 |
| DiceUI | Radix 的二次封装，你已直接用 Radix |
| Aceternity UI | 纯营销/落地页组件，零数据展示模式 |
| Motion Primitives | 主要是文字动画，Framer Motion 文档已覆盖 |
| Hover.dev | 标准 hover 效果，无独特模式 |
| dotUI | 尚未发布，无实际内容 |
| shuffle.dev | 模板级别，非组件级别 |
| Flowbite | 泛用 Bootstrap 风格，不适合金融终端 |
| Float UI | 泛用落地页组件 |
| gluestack-ui | React Native 为主 |

### 10.5 可借鉴的通用 CSS 技法总结

```css
/* 1. 伪元素边缘高光（Origin UI）— 给卡片/输入框添加微妙立体感 */
before:pointer-events-none before:absolute before:inset-0
before:rounded-[calc(var(--radius-xl)-1px)]
before:shadow-[0_1px_rgba(0,0,0,0.04)]

/* 2. 语义状态底色（Origin UI / Tremor）— 用 8% 不透明度替代实心背景 */
bg-market-up-medium/8 text-market-up-medium     /* 涨标签 */
bg-market-down-medium/8 text-market-down-medium  /* 跌标签 */

/* 3. 按钮按压深度（Origin UI）— 通过翻转 inset shadow 创造按压感 */
/* 静止态：顶部高光 */
shadow-[inset_0_1px_rgba(255,255,255,0.16)]
/* 按下态：顶部阴影 */
active:shadow-[inset_0_1px_rgba(0,0,0,0.08)]

/* 4. Container Query 响应式（shadcn blocks）— 根据容器宽度而非视口宽度响应 */
@container/main
@xl/main:grid-cols-3
...
## 11. 详情面板规范（Detail Panel Composition）
