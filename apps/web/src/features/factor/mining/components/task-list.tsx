...
export function TaskList({
  ...
  return (
    <div
      data-slot="task-list"
      className={cn(
        'flex flex-col bg-white shadow-sm border border-mine-border rounded-xl overflow-hidden',
        className,
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-mine-border/50 shrink-0">
        <span className="text-xs font-medium text-mine-muted uppercase tracking-wider">
          挖掘任务
        </span>
        <button
          onClick={onRefresh}
          className="p-1 rounded hover:bg-mine-bg transition-colors text-mine-muted hover:text-mine-text"
          title="刷新"
        >
