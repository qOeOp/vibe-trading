...
// Canvas 2D context: resolved at render time via resolveToken()
import { resolveToken } from '@/lib/chart-tokens';
    ...
    if (spans.length >= 2) {
      ...
      if (firstIdx >= 0) {
        ...
        if (
          ...
          last.endMonth === 11 &&
          first.startMonth === 0 &&
          last.strategyId === first.strategyId &&
          last !== first
        ) {
...
export function PolarRing({
  ...
  useEffect(() => {
    ...
    // Resolve CSS tokens for canvas 2D context
    const BASE_RING_BG = resolveToken('mine-bg');
    const PILLAR_EVEN = resolveToken('mine-border-light');
    const PILLAR_ODD = resolveToken('mine-hover');
    const RING_BORDER = resolveToken('mine-border');
    const CENTER_BG = resolveToken('mine-hover');
    const SELECTED_ARC_COLOR = resolveToken('mine-accent-indigo');
    const TEXT_COLOR = resolveToken('mine-text');
    const MUTED_COLOR = resolveToken('mine-muted');
    ...
    for (const span of mergedArcs) {
      ...
      if (span.startMonth <= span.endMonth) {
        ...
        const firstSectorStart = -Math.PI / 2 + span.startMonth * pillarSlice;
        const lastSectorEnd =
          -Math.PI / 2 + (span.endMonth + 1) * pillarSlice + Math.PI * 2;
        arcStartAngle = firstSectorStart + padA;
        arcEndAngle = lastSectorEnd - padA;
      }
      ...
      const isHighlighted =
        hoverStrategyId === span.strategyId ||
        selectedStrategyId === span.strategyId;
      const hasActiveStrategy =
        hoverStrategyId != null || selectedStrategyId != null;
      if (hasActiveStrategy) {
        alpha = isHighlighted ? 1.0 : 0.1;
      }

      if (selectedMonth !== null) {
        const coversSelected =
          span.startMonth <= span.endMonth
            ? selectedMonth >= span.startMonth && selectedMonth <= span.endMonth
            : selectedMonth >= span.startMonth ||
              selectedMonth <= span.endMonth;
        if (!coversSelected) {
          alpha *= 0.55;
        }
      }
    ...
    if (activeM !== null) {
      ...
      [sStart, sEnd].forEach((a) => {
        ctx.beginPath();
        ctx.arc(
          cx + Math.cos(a) * indicR,
          cy + Math.sin(a) * indicR,
          1.5,
          0,
          Math.PI * 2,
        );
    ...
    ctx.textAlign = 'left';
    ctx.fillStyle =
      yearIdx < availableYears.length - 1
        ? hoverCenter === 'right'
          ? TEXT_COLOR
          : MUTED_COLOR
        : RING_BORDER;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (d >= innerR && d <= outerR + 10) {
        ...
        if (monthData) {
          const ranking = monthData.rankings.find(
            (r) => r.rank === clampedRing + 1,
          );
          if (ranking) {
