...
const QUANTILE_COLORS = [
  {
    ...
    light: 'var(--color-market-down-light)',
  }, // Q1 - worst (green/跌)
  {
    id: 'q2',
    color: 'var(--color-market-down-light)',
    light: 'var(--color-market-down-light)',
  }, // Q2
  {
    id: 'q3',
    color: 'var(--color-market-flat)',
    light: 'var(--color-market-flat)',
  }, // Q3 - neutral
  {
    id: 'q4',
    color: 'var(--color-market-up-light)',
    light: 'var(--color-market-up-light)',
  }, // Q4
  {
    id: 'q5',
...
const COLORS = {
  grid: 'var(--color-mine-border)',
  axis: 'var(--color-mine-muted)',
  text: 'var(--color-mine-text)',
    ...
    chartData.forEach((d, i) => {
      ...
      labelGroup
        ...
        .attr(
          'fill',
          d.value >= 0
            ? 'color-mix(in srgb, var(--color-market-up) 8%, white)'
            : 'color-mix(in srgb, var(--color-market-down) 8%, white)',
        )
        .attr(
          'stroke',
          d.value >= 0
            ? 'color-mix(in srgb, var(--color-market-up) 25%, white)'
            : 'color-mix(in srgb, var(--color-market-down) 25%, white)',
        )
        .attr('stroke-width', 1);
      ...
      bar
        .on('mouseenter', (event) => {
          ...
          const tooltipContent = `
            <div style="font-weight: 600; margin-bottom: 6px; color: var(--color-chart-tooltip-text); font-size: 12px;">
              ${d.quantile}
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <span style="display: flex; align-items: center; gap: 6px;">
                <span style="width: 10px; height: 10px; background: ${qc.color}; border-radius: 50%;"></span>
                <span style="color: var(--color-chart-tooltip-label); font-size: 11px;">${period} Return</span>
              </span>
              <span style="font-weight: 600; color: var(--color-chart-tooltip-text); font-size: 12px; font-variant-numeric: tabular-nums;">
                ${d.value.toFixed(3)}%
              </span>
            </div>
          `;
        ...
        .on('mouseleave', () => {
          bar.transition().duration(150).attr('filter', 'url(#bar-shadow)');
