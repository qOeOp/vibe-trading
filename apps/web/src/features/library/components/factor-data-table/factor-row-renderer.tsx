...
import { GitCompare, Download, ArrowRightLeft, X } from 'lucide-react';
...
import {
  exportAsCSV,
  exportAsJSON,
} from '@/features/library/utils/export-factors';
...
function InlineBatchToolbar({ factors }: { factors: Factor[] }) {
  ...
  return (
    <>
      ...
      <div
        className="inline-flex items-center gap-0.5 ml-2 shrink-0 glass-light border border-white/60 shadow-[0_2px_8px_rgba(0,0,0,0.06)] rounded-full px-1.5 py-0.5"
...
export function FactorRowRenderer({
  ...
  const leafColumnCount = rows[0] ? rows[0].getVisibleCells().length : 1;

  // Determine which row shows the inline toolbar
  const showToolbarForId = selectedFactorIds.size >= 2 ? lastClickedId : null;
