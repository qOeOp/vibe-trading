...
import { PanelSection, PanelKV } from '@/components/shared/panel';
    ...
    <PanelSection noBorder>
      ...
      {open && (
        <div className="grid grid-cols-2 gap-x-4 gap-y-0 mt-2 px-1">
          <PanelKV
            label="IC均值"
            value={fmtIC(d.icMean)}
            color={d.icMean >= 0 ? 'positive' : 'negative'}
          />
          <PanelKV label="IC标准差" value={d.icStd.toFixed(4)} />
          <PanelKV label="正值次数" value={`${d.icPositiveCount}`} />
          <PanelKV label="负值次数" value={`${d.icNegativeCount}`} />
          <PanelKV
            label="显著比例"
            value={`${(d.icSignificantRatio * 100).toFixed(1)}%`}
          />
          <PanelKV
            label="正显著比例"
            value={`${(d.icPositiveSignificantRatio * 100).toFixed(1)}%`}
          />
          <PanelKV
            label="负显著比例"
            value={`${(d.icNegativeSignificantRatio * 100).toFixed(1)}%`}
          />
          <PanelKV
            label="P值"
          ...
          />
          <PanelKV label="偏度" value={d.icSkewness.toFixed(2)} />
          <PanelKV label="峰度" value={d.icKurtosis.toFixed(2)} />
        </div>
      )}
    </PanelSection>
  );
