...
export function WorkflowSection() {
  const lifecycleStages: LifecycleStage[] = [
    {
      ...
      label: '草稿',
      desc: '因子假设已定义，尚未检验',
      color: 'var(--color-mine-muted)',
    },
    {
      stage: 'Testing',
      label: '检验中',
      desc: '正在跑 IC/IR/分位收益检验',
      color: 'var(--color-mine-accent-yellow)',
    },
    {
      stage: 'Validated',
      label: '已验证',
  ...
  const researchSteps: ResearchStep[] = [
    {
      step: '1',
      text: '经济学假设 — 为什么这个因子应该有效？要有逻辑，不是数据挖掘',
      color: 'var(--color-violet-500, #8b5cf6)',
    },
    {
      step: '2',
      text: '量化定义 — 用公式/代码表达因子 + 数据可行性检查',
      color: 'var(--color-mine-accent-teal)',
    },
    {
      step: '3',
      text: '统计检验 — IC > 0.03 且稳定？分位收益单调？换手率合理？',
      color: 'var(--color-mine-accent-yellow)',
    },
    {
      step: '4',
      text: '条件检验 — 不同市场状态 (牛/熊/震荡) 下的因子效力分别如何？',
      color: 'var(--color-pink-500, #ec4899)',
    },
    {
      step: '5',
      text: '正交检验 — 和已知因子是否冗余？增量 IC 贡献多少？',
      color: 'var(--color-blue-500, #3b82f6)',
    },
    {
      step: '6',
      text: '组合回测 — Sharpe / MaxDD / Sortino / Calmar',
      color: 'var(--market-up-medium)',
    },
    {
      step: '7',
      text: '换手约束回测 — 考虑 T+1、佣金+印花税+滑点后的真实收益',
      color: 'var(--color-slate-500, #64748b)',
    },
    {
      step: '8',
      text: '容量分析 — 策略能容纳多少资金？小盘因子可能只适合小资金',
      color: 'var(--color-mine-accent-yellow)',
    },
    {
      step: '9',
      text: '模拟盘 — 真实数据 forward test 至少 1-3 个月',
      color: 'var(--color-mine-accent-teal)',
    },
  ];
  ...
  const dailySteps: DailyStep[] = [
    {
      time: '07:00',
      text: '信息扫描 — 隔夜政策 · 全球映射 · A50期指 · 舆情',
      module: 'Market',
    },
    {
      time: '08:00',
      text: 'Dashboard — 策略隔夜重算结果 · 今日信号 · 仓位计算 · 风险预检',
      module: 'Dashboard',
    },
    {
      time: '09:15',
      text: '集合竞价 — 三段式观察: 可撤单→不可撤单→撮合定价 · 发现预期差',
      module: 'Market',
      highlight: true,
    },
    {
      time: '09:30',
      text: '开盘执行 — 按信号下单 · 涨停板策略 · 大单拆分',
      module: 'Trading',
    },
    {
      time: '盘中',
      text: 'Risk — 持仓 P&L · 止损触发 · 板块轮动 · 北向资金 · 异常告警',
      module: 'Risk',
    },
    {
      time: '11:30',
      text: '午休 — 复盘上午执行 · 调整下午计划（A 股特有 1.5h 窗口）',
      module: 'Journal',
      highlight: true,
    },
    {
      time: '14:57',
      text: '尾盘集合竞价 — 深市特有 · T+1 约束下的仓位调整',
      module: 'Trading',
      highlight: true,
    },
    {
      time: '15:00',
      text: '收盘 → 龙虎榜 · 大宗交易数据获取',
      module: 'Market',
    },
    {
      time: '15:30',
      text: '自动任务 — 数据拉取→因子重算→信号更新→风险指标重算',
      module: 'Auto',
    },
    {
      time: '晚间',
      text: 'Journal — 执行偏差 · 情绪标注 · 信号质量回顾 · 研报阅读',
      module: 'Journal',
    },
  ];
  ...
  const ensembleMethods: EnsembleMethod[] = [
    {
      name: '等权投票',
      desc: '多策略打分取平均。稳定但 alpha 被稀释',
      rec: false,
    },
    {
      name: '信号共振',
      desc: '多策略一致时才交易。高确信但机会少',
      rec: false,
    },
    {
      name: '动态加权',
      desc: '按长期 IC 表现分配权重。自适应但需防过拟合',
      rec: true,
    },
  ];
    ...
    <div className="p-4 space-y-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-4 mt-1">
            <span className="text-[10px] text-mine-muted">
              <strong className="text-mine-text">T+1</strong>{' '}
              当日买入不能当日卖出
            </span>
            <span className="text-[10px] text-mine-muted">
              <strong className="text-mine-text">涨跌停</strong> 主板±10% /
              创业板科创板±20%
            </span>
            <span className="text-[10px] text-mine-muted">
              <strong className="text-mine-text">北向资金</strong>{' '}
              最重要的聪明钱指标
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card variant="frosted">
          <CardHeader title="慢循环 — 研究周期" subtitle="周/月级别" />
          <CardContent className="px-3 pb-3 space-y-2">
            <p className="text-[11px] text-mine-text leading-[1.6]">
              花数周打磨一个策略，反复检验直到通过所有统计门槛才允许上线。类比：训练
              ML 模型。
            </p>
            {researchSteps.map((s) => (
              <div
                key={s.step}
                className="flex items-start gap-2 p-2 rounded-lg bg-mine-bg"
              >
                <span
        ...
        <Card>
          <CardHeader title="快循环 — 执行周期" subtitle="每个交易日" />
          <CardContent className="px-3 pb-3 space-y-2">
            <p className="text-[11px] text-mine-text leading-[1.6]">
            ...
            {dailySteps.map((s) => (
              ...
              >
                ...
                <span className="text-[11px] text-mine-text flex-1">
                  {s.text}
                </span>
                <span className="px-1.5 py-0.5 rounded font-mono text-[9px] bg-mine-accent-teal/10 text-mine-accent-teal">
                  {s.module}
                </span>
              </div>
            ))}
      ...
      {/* Strategy Lifecycle */}
      <div>
        <h3 className="font-semibold mb-2 px-1 text-[13px] text-mine-text">
          策略生命周期
        </h3>
        <Card variant="frosted">
          <div className="p-4">
              ...
              {lifecycleStages.map((s, i) => (
                <div key={s.stage} className="flex items-center flex-1">
                  <div className="flex flex-col items-center text-center flex-1 min-w-0">
                    ...
                    <span className="font-semibold text-[10px] text-mine-text">
                      {s.label}
                    </span>
                    <span
            ...
            <div className="mt-3 pt-3 space-y-1.5 border-t border-mine-border">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-mine-text font-medium">
                  门禁规则：Factor Lab (研究区) → Portfolio (生产环境)
                  必须经过显式部署
                </span>
              </div>
              <p className="text-[10px] text-mine-muted pl-[22px]">
                只有 Live 状态的策略才会出现在 Portfolio 和 Trading
                模块中。未验证的策略永远不会碰到真钱。
              </p>
            </div>
          </div>
      ...
      <Card>
        <CardHeader title="多策略集成" subtitle="Signal Aggregation" />
        <CardContent className="px-3 pb-3 space-y-2">
          <p className="text-[11px] text-mine-muted leading-[1.6]">
            同一时间可有多个策略在不同阶段并行。Live
            策略的信号通过集成方式聚合为最终交易建议：
          </p>
            ...
            {ensembleMethods.map((m) => (
              ...
              >
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  {m.rec && (
                    <span className="px-1 py-0.5 rounded font-medium text-[8px] bg-mine-accent-teal/10 text-mine-accent-teal">
                      推荐
                    </span>
                  )}
                </div>
                <p className="text-[10px] text-mine-muted leading-[1.4]">
                  {m.desc}
                </p>
              </div>
            ))}
      ...
      <Card variant="frosted">
        ...
        <CardContent className="px-3 pb-3">
            ...
            <div>
              ...
              <div className="mt-1.5 space-y-1">
                {[
                  ...
                  <div
                    key={t}
                    className="flex items-center gap-1.5 text-[10px] text-mine-muted"
                  >
          ...
          >
            <span className="text-[10px] text-mine-accent-yellow">
              &#x26A0; Journal 的目的是「帮你克制住乱改策略的冲动」—
              记录和观察，而不是每天动手调参数。只有累积了足够统计证据才触发策略调整。
            </span>
          </div>
        </CardContent>
      </Card>
