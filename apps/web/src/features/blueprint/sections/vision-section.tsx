...
export function VisionSection() {
  ...
  return (
    <div className="space-y-4 p-4">
      {/* Hero card */}
      <Card variant="frosted" style={{ padding: 0 }}>
        <CardContent className="p-5">
          <div className="mb-3 flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-mine-accent-teal/10 text-mine-accent-teal">
          ...
          <p className="text-[13px] leading-[1.7] text-mine-text">
            ...
            从数据采集、因子研究、策略回测、到交易执行和复盘进化的完整
            pipeline， 面向 A 股市场，为需要 truth、speed、clarity
            的专业量化交易者打造。 UI 服务于逻辑，而非反过来。
          </p>
        </CardContent>
      </Card>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(
            ...
            >
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-market-up/[0.06] text-market-up">
        ...
        <Card className="flex-1">
          ...
          <div className="space-y-1.5 px-3 pb-3">
            {(
              ...
              >
                <div
                  className={`h-1.5 w-1.5 rounded-full ${i === 0 ? 'bg-mine-accent-teal' : 'bg-mine-border'}`}
                />
                {t}
                {i === 0 && (
                  <span className="rounded px-1.5 py-0.5 text-[9px] font-medium bg-mine-accent-teal/10 text-mine-accent-teal">
                    MVP
                  </span>
                )}
              </div>
        ...
        <Card className="flex-1">
          ...
          <div className="space-y-1.5 px-3 pb-3">
            {(
              ...
              >
                <div
                  className={`h-1.5 w-1.5 rounded-full ${i === 2 ? 'bg-mine-accent-teal' : 'bg-mine-border'}`}
                />
                {t.text}
              </div>
            ))}
