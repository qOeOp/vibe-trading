...
export function RoadmapSection() {
  return (
    <div className="p-4 space-y-4">
      ...
      {PHASES.map((p) => (
        <Card key={p.phase} className="overflow-visible">
          <div className="p-4">
            <div className="flex items-center gap-3 mb-3">
              <span
                className="font-mono font-bold px-2.5 py-1 rounded-lg text-[11px]"
                style={{ background: `${p.color}15`, color: p.color }}
              >
                {p.phase}
              </span>
              <span className="font-semibold text-sm text-mine-text">
                {p.title}
              </span>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1">
              {p.tasks.map((t, i) => (
                <div key={i} className="flex items-center gap-2 py-1">
                  <div
                    className={`w-4 h-4 rounded flex items-center justify-center shrink-0 ${t.done ? 'bg-mine-accent-teal/10' : 'bg-mine-bg'}`}
                  >
                    {t.done ? (
                  ...
                  <span
                    className={`text-[11px] ${t.done ? 'text-mine-text' : 'text-mine-muted'}`}
                  >
                    {t.text}
                  </span>
                </div>
