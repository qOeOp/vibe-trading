...
export class TableContextProvider extends AIContextProvider<TableContextItem> {
  ...
  private createTableInfoElement(
    ...
    if (table.source) {
      ...
      sourceDiv.classList.add('text-xs', 'text-mine-muted');
    ...
    if (shape) {
      ...
      shapeDiv.classList.add('text-xs', 'bg-mine-hover', 'px-2', 'py-1', 'rounded');
    ...
    if (table.columns && table.columns.length > 0) {
      ...
      headerRow.classList.add('bg-mine-hover');
      ...
      table.columns.forEach((column, index) => {
        ...
        row.classList.add(index % 2 === 0 ? 'bg-mine-page-bg' : 'bg-mine-hover/30');
        ...
        typeCell.classList.add('p-2', 'text-mine-muted');
