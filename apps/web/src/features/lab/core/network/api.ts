...
export const API = {
  ...
  get<RESP = null>(
    ...
    opts: {
      headers?: Record<string, string>;
      baseUrl?: string;
      /** If true, suppress Logger.error on failure (for optional endpoints like LSP) */
      silent?: boolean;
    } = {},
  ): Promise<RESP> {
    const baseUrl = Strings.withTrailingSlash(
...
export function createClientWithRuntimeManager(runtimeManager: RuntimeManager) {
  const marimoClient = createMarimoClient({
    // Strip query params from base URL — openapi-fetch uses string concatenation,
    // so ?file=...&server_token=... would corrupt API paths like /api/kernel/run
    baseUrl: (() => {
      const url = new URL(runtimeManager.httpURL.toString());
      url.search = '';
      url.hash = '';
      return url.toString();
    })(),
