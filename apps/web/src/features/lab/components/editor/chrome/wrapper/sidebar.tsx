...
export const Sidebar: React.FC = () => {
    ...
    return (
      <div className="flex flex-col gap-2 pt-3 pl-2 z-50 print:hidden">
        {sidebarItems.map((panel) => (
          ...
          >
            <button
              onClick={() => toggleApplication(panel.type)}
              className={cn(
                'flex items-center justify-center h-[27px] w-[27px] rounded-full',
                'border border-foreground/10 shadow-xs-solid active:shadow-none',
                'transition-colors cursor-pointer',
                selectedPanel === panel.type && isSidebarOpen
                  ? 'bg-mine-nav-active text-white'
                  : 'bg-mine-nav-active/90 text-white/90 hover:bg-mine-nav-active hover:text-white',
              )}
            >
              {panel.type === 'errors' ? (
  ...
  // ── Default mode: vertical sidebar strip with reorderable list ──
  return (
