/**
 * @fileoverview Normalized area chart component
 *
 * @migrated-from ngx-charts
 * @source https://github.com/swimlane/ngx-charts
 * @commit e241d94221430000a8fddaa9eb4d8f2878ed8a38
 * @original-file projects/swimlane/ngx-charts/src/lib/area-chart/area-chart-normalized.component.ts
 *
 * @description
 * Normalized (100%) stacked area chart where each vertical slice totals to 100%.
 * Useful for showing relative proportions over time.
...
export interface AreaChartNormalizedProps {
...
/**
 * Normalized Area Chart Component
 *
 * 100% stacked area chart where each vertical slice is normalized
 * to show percentage contributions (0-100%).
 */
export function AreaChartNormalized({
  data,
  width,
  height,
  colorScheme = 'cool',
  ...
  return (
    <BaseChart
      ...
      colors={customColors}
      animated={animated}
      className={`area-chart-normalized ${className}`}
    >
      {(dimensions) => (
        <AreaChartNormalizedContent
          data={data}
...
interface AreaChartNormalizedContentProps {
  data: MultiSeries;
...
function AreaChartNormalizedContent({
  data,
  dimensions,
  colorScheme,
  schemeType,
  ...
  maxYAxisTickLength,
  wrapTicks,
}: AreaChartNormalizedContentProps) {
  const {
    dims,
    xScale,
    yScale,
  ...
  // Custom tooltip template that shows percentage
  const normalizedTooltipTemplate = useCallback(
    (items: TooltipItem[]) => {
      if (tooltipTemplate) {
        return tooltipTemplate(items);
      }

      return (
        <div style={{ padding: '5px 0', pointerEvents: 'none' }}>
          {items.map((item, index) => (
            ...
            >
              ...
              />
              {item.series}: {typeof item.value === 'number' ? `${item.value.toFixed(2)}%` : item.value}
            </div>
          ))}
        </div>
      );
      ...
      <g transform={transform} className="area-chart chart">
        ...
        {yAxis.visible && (
          <YAxis
            ...
            tickFormatting={yAxis.tickFormatting || ((v: number) => `${v}%`)}
            ticks={yAxis.ticks}
            wrapTicks={wrapTicks}
          ...
          {!tooltip.disabled && (
            <g onMouseLeave={handleMouseLeave}>
              <TooltipArea
                ...
                tooltipTemplate={normalizedTooltipTemplate}
                onHover={handleHover}
              />
            </g>
          )}
